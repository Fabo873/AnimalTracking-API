...
class DestinationModel(db.Model):
    ...
    @classmethod
    def find_by_name(cls, name: str) -> DestinationModel:
        return cls.query.filter_by(name=name).first()

    @classmethod
    def find_by_attributes(cls, name: str = None, limit: int = None, offset: int = None) -> list:
        destination = cls.query
        if name:
            search = "%{}%".format(name)
            destination = destination.filter(cls.name.like(search))
        if limit:
    ...
    @classmethod
    def find_by_id(cls, _id: int) -> DestinationModel:
